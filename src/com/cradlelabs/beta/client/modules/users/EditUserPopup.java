...
public class EditUserPopup extends Composite {

	private int stageIndex = 0;
	EditUserPopupEventHandler handler;
	...
	public interface EditUserPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, EditUserPopup> {
	}
	...
	public EditUserPopup(UserModel model){
	...
	private void initComponent(UserModel model){
		titleSpan.setInnerText("Edit User");
		wizard = new EditStageOne(model);
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("User Updated Successfully");
	}
	...
	private void doSaveAgent(UserModel model){
		...
		GlobalResources.getInstance().getUpdateRPC().updateUser(model, new AsyncCallback<Integer>() {
	...
	public void setEditUserPopupEventHandler(EditUserPopupEventHandler handler){
